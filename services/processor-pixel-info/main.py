"""Cloud Functions service that processes Pub/Sub messages for pixel_info command."""
import json
import base64
...
logger, tracing = init_observability('processor-pixel-info', app=None)


@cloud_event
@traced_function("processor_pixel_info_handler")
def processor_pixel_info_handler(cloud_event: CloudEvent):
    ...
    try:
        ...
        if response:
            logger.info("Pixel_info command processed, sending response", correlation_id=correlation_id, command_name=command_name)
        else:
            logger.warning("No response generated for pixel_info command", correlation_id=correlation_id, command_name=command_name)

        handle_processor_response(interaction_data, response, logger)

        logger.info("Pixel_info message processed successfully", correlation_id=correlation_id, command_name=command_name, event_id=event_id)

    except Exception as e:
        logger.error("Critical error in processor_pixel_info_handler", error=e, correlation_id=correlation_id)
        raise
