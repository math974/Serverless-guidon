"""Cloud Functions service that processes Pub/Sub messages for stats command."""
import json
import base64
...
logger, tracing = init_observability('processor-stats', app=None)


@cloud_event
@traced_function("processor_stats_handler")
def processor_stats_handler(cloud_event: CloudEvent):
    ...
    try:
        ...
        if response:
            logger.info("Stats command processed, sending response", correlation_id=correlation_id, command_name=command_name)
        else:
            logger.warning("No response generated for stats command", correlation_id=correlation_id, command_name=command_name)

        handle_processor_response(interaction_data, response, logger)

        logger.info("Stats message processed successfully", correlation_id=correlation_id, command_name=command_name, event_id=event_id)

    except Exception as e:
        logger.error("Critical error in processor_stats_handler", error=e, correlation_id=correlation_id)
        raise
