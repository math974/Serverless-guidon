"""Cloud Functions service that processes Pub/Sub messages for colors command."""
import json
import base64
...
logger, tracing = init_observability('processor-colors', app=None)


@cloud_event
@traced_function("processor_colors_handler")
def processor_colors_handler(cloud_event: CloudEvent):
    ...
    try:
        ...
        if response:
            logger.info("Colors command processed, sending response", correlation_id=correlation_id, command_name=command_name)
        else:
            logger.warning("No response generated for colors command", correlation_id=correlation_id, command_name=command_name)

        handle_processor_response(interaction_data, response, logger)

        logger.info("Colors message processed successfully", correlation_id=correlation_id, command_name=command_name, event_id=event_id)

    except Exception as e:
        logger.error("Critical error in processor_colors_handler", error=e, correlation_id=correlation_id)
        raise
