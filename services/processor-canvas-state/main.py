"""Cloud Functions service that processes Pub/Sub messages for canvas_state command."""
import json
import base64
...
logger, tracing = init_observability('processor-canvas-state', app=None)


@cloud_event
@traced_function("processor_canvas_state_handler")
def processor_canvas_state_handler(cloud_event: CloudEvent):
    ...
    try:
        ...
        logger.info(
            "Processing canvas_state command",
        ...
        if response:
            logger.info("Canvas_state command processed, sending response", correlation_id=correlation_id, command_name=command_name)
        else:
            logger.warning("No response generated for canvas_state command", correlation_id=correlation_id, command_name=command_name)

        handle_processor_response(interaction_data, response, logger)

        logger.info("Canvas_state message processed successfully", correlation_id=correlation_id, command_name=command_name, event_id=event_id)

    except Exception as e:
        logger.error("Critical error in processor_canvas_state_handler", error=e, correlation_id=correlation_id)
        raise
