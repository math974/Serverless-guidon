"""
services/processor-art/canvas_manager.py
...
logger, _ = init_observability('processor-art-canvas', app=None)
...
class CanvasManager:
    ...
    CANVAS_SIZE = 48  # 48x48 pixels
    DEFAULT_COLOR = "#FFFFFF"  # White background
    PIXEL_SCALE = 10  # Each canvas pixel = 10x10 image pixels
    ...
    def get_recent_snapshots(self, limit: int = 10) -> List[Dict]:
        ...
        try:
            query = self.snapshots_ref.order_by(
                'created_at',
            ...
            for doc in query.stream():
                data = doc.to_dict()
                if 'canvas_state_json' in data:
                    del data['canvas_state_json']
                snapshots.append(data)

            return snapshots
        except Exception as e:
            logger.error("Error getting snapshots", error=e)
            return []

    def get_snapshot(self, snapshot_id: str) -> Optional[Dict]:
        """Get a specific snapshot with full data."""
        try:
            doc = self.snapshots_ref.document(snapshot_id).get()
            if doc.exists:
                return doc.to_dict()
            return None
        except Exception as e:
    ...
    def get_leaderboard(self, limit: int = 10) -> List[Dict]:
        """Get top contributors by pixel count."""
        try:
            user_counts = {}

            pixels = self.pixels_ref.stream()
            ...
            for doc in pixels:
                ...
                if user_id:
                    ...
                    user_counts[user_id]['pixel_count'] += 1
    ...
    def clear_canvas(self, admin_user_id: str = None) -> Dict:
        """
        Clear the entire canvas (admin only).
        Creates a backup snapshot before clearing.
        """
        try:
            backup = self.create_snapshot(
                user_id=admin_user_id,
                username="System (Canvas Clear Backup)"
            )
            ...
            self.canvas_ref.set({
                ...
                'backup_snapshot': backup['snapshot_id']
            })

            return {
                'success': True,
                'pixels_cleared': count,
                'backup_snapshot': backup['snapshot_id']
            }

        except Exception as e:
            logger.error("Error clearing canvas", error=e)
            return {
                'success': False,
                'error': str(e)
            }
